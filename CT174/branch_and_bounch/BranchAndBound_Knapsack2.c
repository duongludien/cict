/*******************************************************************\
* Problem:		The Knapsack 2 (Every item Every item is only one)	*
* Algorithm:	Branch and Bound									*
* Input file:	BranchAndBound_Knapsack2.txt						*
...
void ReadFromFile() {
	int i = 0;
	char *tmp;
	FILE *f = fopen("BranchAndBound_Knapsack2.txt", "r");
	fscanf(f, "%f", &W);
...
void inDSDV() {
	...
	for(i=0; i<n; i++) 
		if(dsdv[i].PA > 0) {
			printf("| %3d | %25s | %8d | %10.1f |\n", stt, dsdv[i].ten, dsdv[i].PA, dsdv[i].GT);
			printf("|-----|---------------------------|----------|------------|\n");
			stt++;
	...
	printf("Tong trong luong va Tong gia tri sau khi da chon: %.1f va %.1f\n", ttl, GTLNTT);
}
...
void BranchAndBound(int i) {
	...
	for(j=min(1,TLCL/dsdv[i].TL); j>=0; j--) {
		TGT += dsdv[i].GT * j;
		TLCL -= dsdv[i].TL * j;
		if(i < n-1)
		...
		if(CT > GTLNTT) {
			x[i] = j;
			if(TLCL == 0.0 || i == n-1)		//Balo het cho OR Da xet het do vat
				GhiNhanKyLuc();
			else 
				BranchAndBound(i+1);
		}
