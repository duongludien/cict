...
* Algorithm:	Greedy												*
* Input file:	Greedy_Knapsack1.txt								*
...
DoVat* ReadFromFile(int *n, int *W) {
	FILE *f = fopen("Greedy_Knapsack1.txt", "r");
	DoVat *dsdv;
	char *tmp;
	int i = 0;
...
void Greedy(DoVat *dsdv, int n, int W, int *TV, int *TW) {
	...
	*TW = 0;		//Total weight
	Sorting(dsdv, n);
	while(i<n && W > 0) {	// for(i=0; i<n && W > 0; i++)
...
void inDanhSach(DoVat *dsdv, int n) {
	...
	printf("\n|-----|---------------------------|----------|------------|------------|-------------|------------|\n");
	printf("| %3s | %25s | %8s | %10s | %10s | %11s | %10s |\n", "STT", "Loai do vat", "So luong", "Gia tri", "Tong GT", "Trong luong", "Tong TL");
	printf("|-----|---------------------------|----------|------------|------------|-------------|------------|\n");
	for(i=0; i<n; i++) 
		if(dsdv[i].PA > 0) {
			printf("| %3d | %25s | %8d | %10d | %10d | %11d | %10d |\n", stt, dsdv[i].ten, dsdv[i].PA, dsdv[i].GT, dsdv[i].PA * dsdv[i].GT,  dsdv[i].TL, dsdv[i].PA * dsdv[i].TL);
			printf("|-----|---------------------------|----------|------------|------------|-------------|------------|\n");
			stt++;
		}
}
