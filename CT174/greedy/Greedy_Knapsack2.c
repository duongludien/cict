/*******************************************************************\
* Problem:		The Knapsack 2 (Every item is only one)				*
* Algorithm:	Greedy												*
* Input file:	Greedy_Knapsack2.txt								*
...
DoVat* ReadFromFile(int *n, int *W) {
	FILE *f = fopen("Greedy_Knapsack2.txt", "r");
	DoVat *dsdv;
	char *tmp;
	int i = 0;
...
void inDanhSach(DoVat *dsdv, int n) {
	...
	printf("\n|-----|---------------------------|----------|------------|-------------|\n");
	printf("| %3s | %25s | %8s | %10s | %11s |\n", "STT", "Loai do vat", "So luong", "Gia tri", "Trong luong");
	printf("|-----|---------------------------|----------|------------|-------------|\n");
	for(i=0; i<n; i++) 
		if(dsdv[i].PA >= 0) {
			printf("| %3d | %25s | %8d | %10d | %11d |\n", stt, dsdv[i].ten, dsdv[i].PA, dsdv[i].GT,  dsdv[i].TL);
			printf("|-----|---------------------------|----------|------------|-------------|\n");
			stt++;
		}
}
